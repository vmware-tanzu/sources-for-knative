...
// controller reconciling v1alpha1.VSphereSource.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.VSphereSource. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.VSphereSource) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.VSphereSource.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.VSphereSource. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.VSphereSource) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.VSphereSource if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.VSphereSource.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.VSphereSource) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.VSphereSource if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1alpha1.VSphereSource.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1alpha1.VSphereSource) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.VSphereSource) reconciler.Event
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister sourcesv1alpha1.VSphereSourceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.VSphereSources(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.VSphereSource, desired *v1alpha1.VSphereSource) error {
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.VSphereSource) (*v1alpha1.VSphereSource, error) {

	getter := r.Lister.VSphereSources(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.VSphereSource) (*v1alpha1.VSphereSource, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.VSphereSource, reconcileEvent reconciler.Event) (*v1alpha1.VSphereSource, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
