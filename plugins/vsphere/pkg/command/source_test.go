...
func TestNewSourceCommand(t *testing.T) {
	...
	t.Run("defines basic metadata", func(t *testing.T) {
		...
		checkFlag(t, sourceCommand, "sink-service-ref")
		checkFlag(t, sourceCommand, "sink-knative-service-ref")
		checkFlag(t, sourceCommand, "sink-knative-broker-ref")
	...
	t.Run("fails to execute with an empty sink URI and empty sink references", func(t *testing.T) {
		sourceCommand, _ := sourceCommand(regularClientConfig())
		sourceCommand.SetArgs([]string{
			"--name", sourceName,
			"--address", sourceAddress,
		...
		assert.ErrorContains(t, err, `requires a nonempty URI provided with the --sink-uri option,
...
or a non-empty reference to a Knative Broker name with the --sink-knative-broker-ref option`)
	...
	sinkMatrix := []struct {
		...
		{"all", []string{
			"--sink-uri", sinkUri,
			"--sink-service-ref", "some-service",
			"--sink-knative-service-ref", "some-knative-service",
			"--sink-knative-broker-ref", "some-broker"},
		},
		{"all but Knative Broker", []string{
			"--sink-uri", sinkUri,
			"--sink-service-ref", "some-service",
			"--sink-knative-service-ref", "some-knative-service"},
		},
		{"all but Knative Service", []string{
			"--sink-uri", sinkUri,
			"--sink-service-ref", "some-service",
			"--sink-knative-broker-ref", "some-broker"},
		},
		{"all but Service", []string{
			"--sink-uri", sinkUri,
			"--sink-knative-service-ref", "some-knative-service",
			"--sink-knative-broker-ref", "some-broker"},
		},
		{"all but sink URI", []string{
			"--sink-service-ref", "some-service",
			"--sink-knative-service-ref", "some-knative-service",
			"--sink-knative-broker-ref", "some-broker"},
		},
		{"Service and Knative Service", []string{
			"--sink-service-ref", "some-service",
			"--sink-knative-service-ref", "some-knative-service"},
		},
		{"Service and Knative Broker", []string{
			"--sink-service-ref", "some-service",
			"--sink-knative-broker-ref", "some-broker"},
		},
		{"Knative Service and Knative Broker", []string{
			"--sink-knative-service-ref", "some-knative-service",
			"--sink-knative-broker-ref", "some-broker"},
		},
	}
	for _, sinkTestCase := range sinkMatrix {
		t.Run(fmt.Sprintf("fails to execute with %s sink flags set", sinkTestCase.description), func(t *testing.T) {
			...
			assert.ErrorContains(t, err, `sink can optionally be configured with one of the following flags (but several were set):
	...
	t.Run("creates basic source with sink URI in default namespace", func(t *testing.T) {
		sinkUri := "https://example.sink.com"
	...
	t.Run("creates insecure source with Service and relative sink URI in explicit namespace", func(t *testing.T) {
		...
		assert.Equal(t, source.Spec.Sink.URI.String(), sinkUri)
		assertSinkReference(t, source.Spec.Sink.Ref, "v1", "Service", namespace, serviceName)
	})

	t.Run("creates source with Knative Service sink reference in default namespace", func(t *testing.T) {
		sourceCommand, vSphereClientSet := sourceCommand(regularClientConfig())
		knativeServiceName := "some-knative-service"
		sourceCommand.SetArgs([]string{
			"--name", sourceName,
			"--address", sourceAddress,
			"--secret-ref", secretRef,
			"--sink-knative-service-ref", knativeServiceName,
		})
		...
		source := retrieveCreatedSource(t, err, vSphereClientSet, defaultNamespace, sourceName)
		assertBasicSource(t, &source.Spec, sourceAddress, secretRef, false)
		assert.Check(t, source.Spec.Sink.URI == nil)
		assertSinkReference(t, source.Spec.Sink.Ref, "serving.knative.dev/v1", "Service", defaultNamespace, knativeServiceName)
	})

	t.Run("creates source with Knative Broker sink reference and relative sink URI in default namespace", func(t *testing.T) {
		sourceCommand, vSphereClientSet := sourceCommand(regularClientConfig())
		knativeBrokerName := "some-knative-broker"
		sinkUri := "/relative/uri"
		sourceCommand.SetArgs([]string{
			"--name", sourceName,
			"--address", sourceAddress,
			"--secret-ref", secretRef,
			"--sink-uri", sinkUri,
			"--sink-knative-broker-ref", knativeBrokerName,
		})
		...
		assertBasicSource(t, &source.Spec, sourceAddress, secretRef, false)
		assert.Equal(t, source.Spec.Sink.URI.String(), sinkUri)
		assertSinkReference(t, source.Spec.Sink.Ref, "eventing.knative.dev/v1beta1", "Broker", defaultNamespace, knativeBrokerName)
	})
	...
	t.Run("fails to execute when default namespace retrieval fails", func(t *testing.T) {
		...
		assert.Equal(t, err, namespaceError)
	})
